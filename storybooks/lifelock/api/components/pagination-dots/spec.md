...
**Dark appearance** — `--color-text-inverse` (inactive dot fill at 40 % via `color-mix()`), `--color-bg-default` (active pill fill). Substitutes for a dedicated dark-surface inactive-dot semantic alias which LifeLock has not published; see properties table note.
...
- **Dark-mode inactive-dot token substitution.** Same class of substitution as the Checkbox / Radio state-circle wash: the dark-mode inactive dots paint with `--color-text-inverse` at 40 % opacity via `color-mix()`, because LifeLock has no `--color-pagination-dot-inactive-on-dark` (or equivalent) semantic alias. Designer follow-up logged for a dedicated alias.
