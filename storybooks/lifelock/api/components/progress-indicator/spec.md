...
layout mirrors the [LifeLock-mode `.Sticker Sheets/progress-indicator/LifeLock`](https://www.figma.com/design/0o8SL5BEk8wHtgud00dRyg/Web-ODS-Shared-Library?node-id=615-80630&m=dev)
sibling at `615:80630` (the Circular master variant set lives on the same
canvas `539:28423` but the LifeLock sticker sheet is the source of truth for
LifeLock-resolved paint values).
...
- `theme=light` — label / value text paints with the LifeLock
...
LifeLock sticker-sheet diameters verbatim (`xs` 16 px, `sm` 24 px,
...
(the LifeLock-mode-resolved value of the Figma `Progress indicator/base`
...
(the LifeLock-mode-resolved value of `Progress indicator/highlight`). See
§ "Notes & open questions" for the Designer Follow-Up to fold these into
a dedicated `--progress-indicator-*` LifeLock component-knob token surface.
...
paint with `--color-text-primary` (`--color-lifelock-green`).
...
SemiBold (14 px / 16 px / 0.6 px letter-spacing); LifeLock's
typography ramp uses Inter Tight and does not publish a Tagline scale,
so the closest LifeLock equivalent ships in code. A component-knob
...
All five rows resolve to canonical LifeLock typography primitives; no
...
LifeLock's primitive token ramps today (`--border-radius-*` is
role-keyed: control 4 / card 8 / dialog 16 / pill 9999; the Figma
Spec Frame references a `border-radius-xs` primitive that LifeLock
doesn't publish).

**Geometry (circular)** — the ring stroke uses the canonical LifeLock
...
as a LifeLock primitive, so each `.c-progress-indicator--size-<n>`
...
See `storybook-lifelock/src/tokens/colors/_colors.scss`,
`storybook-lifelock/src/tokens/typography/_typography.scss`, and
`storybook-lifelock/src/tokens/borders/_borders.scss` for the
...
- Contrast — both linear `.c-progress-indicator__fill` and circular
  ...
  contrast — light theme paints lifelock-green (`#013638`) on light
...
The `theme=dark` variant exists because the LifeLock product surface
...
- **LifeLock-mode Linear master still resolves to other brand blue.** The
  `Progress indicator — Overview` Spec Frame at `1380:1478` and the
  Linear master variant set at `1380:1255` both render under a
  non-LifeLock theme today — `Progress indicator/light/highlight`
  resolves to `#0009ec` (other brand blue), not a LifeLock token. The
  LifeLock-mode-resolved values used by this spec come from the
  sibling LifeLock-mode sticker sheet for the Circular variant at
  ...
  under `THEME=LifeLock` so the canonical Spec Frame and master
  variant set carry the LifeLock paint values directly, then
  re-sync.
- **No LifeLock-mode sticker sheet for Linear.** The Circular variant
  publishes a `.Sticker Sheets/progress-indicator/LifeLock` frame at
  `615:80630`; Linear does not. Designer Follow-Up — add a LifeLock
  sticker sheet for the Linear master so the AllStyles gallery has
  a canonical Figma frame to mirror per breakpoint.
- **No LifeLock-mode Circular Spec Frame.** The Circular layout's
  reference is the `.Sticker Sheets/progress-indicator/LifeLock`
...
- **Component-knob tokens are component-local.** The four Figma
  ...
  than as a dedicated `storybook-lifelock/src/tokens/progress-indicator/`
...
- **Linear-only token gaps still apply.** `border-radius-xs` (2 px)
  ...
  because LifeLock's `--border-radius-*` ladder is role-keyed
  ...
  LifeLock primitives ramp (which would also remove
...
- **Typography substitute: Tagline / Small SemiBold → Body-sm
  ...
  is not present in LifeLock's Inter Tight ramp. The closest match
  ...
  Follow-Up — confirm whether the LifeLock typography ramp should
...
- **Spec Frame `1380:1478` ↔ spec.md (B) conflicts deliberately
  ...
  - `## Variant axes` — Spec Frame lists Linear-only axes
    ...
    (`xs`/`sm`/`md`/`lg`/`xxl`) for the LifeLock Circular sticker
    sheet fold-in.
  ...
  - `## Tokens consumed` — Spec Frame references multi-brand
    ...
    `space-1` track thickness); spec.md documents the LifeLock
    ...
    LifeLock-mode Spec Frame that carries the LifeLock-resolved
...
- **Spec Frame `1380:1478` is a hand-built `<frame>`, not an
  ...
  template + bootstrap
  `storybook-lifelock/figma-snapshots/template-spec-frame.manifest.json`
...
- [Web-ODS Shared Library : Progress indicator / Linear (master)](https://www.figma.com/design/0o8SL5BEk8wHtgud00dRyg/Web-ODS-Shared-Library?node-id=1380-1255&m=dev) (Linear master variant set — renders under non-LifeLock theme today; see § "Notes & open questions" for the rebake follow-up)
- [Web-ODS Shared Library : .Sticker Sheets / progress-indicator / LifeLock](https://www.figma.com/design/0o8SL5BEk8wHtgud00dRyg/Web-ODS-Shared-Library?node-id=615-80630&m=dev) (Circular LifeLock-mode sticker sheet — canonical source for both the linear paint values and the full circular layout incl. the five `size` stamps XS / S / M / L / XXL)
