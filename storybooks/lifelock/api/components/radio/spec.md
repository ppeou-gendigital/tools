...
|---|---|---|---|
| `commitOn` | `"input" \| "change"` | `"change"` | Which native event fires `lifelock:radio:commit`. |
...
|---|---|---|
| `lifelock:radio:change` | `{ checked: boolean, name: string, value: string }` | Native `change` on the input. |
| `lifelock:radio:commit` | `{ checked: boolean, name: string, value: string, source: "pointer" \| "keyboard" \| "api" }` | After commit per the `commitOn` option. |
...
dispatching the namespaced `lifelock:radio:*` CustomEvents.
