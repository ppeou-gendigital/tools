...
the default theme `inverse-secondary` colour at 20 % opacity. LifeLock
has no dedicated state-wash alias today, so the implementation mixes
`--color-text-secondary` (= `--color-off-black` under LifeLock mode)
at 20 % / 30 % — a near-black wash that paints the same visual role
on the LifeLock surface palette (Designer Follow-Up logged for a
...
**State circle** — `--color-text-secondary` mixed at 20 % / 30 % opacity via `color-mix()` for the 32 × 32 px hover / focus / pressed wash. Substitutes for the default theme `inverse-secondary` source until LifeLock publishes a dedicated state-wash alias.
...
|---|---|---|---|
| `commitOn` | `"input" \| "change"` | `"change"` | Which native event fires `lifelock:checkbox:commit`. |
...
|---|---|---|
| `lifelock:checkbox:change` | `{ selection: "checked" \| "unchecked" }` | Native `change` on the underlying input. |
| `lifelock:checkbox:commit` | `{ selection: "checked" \| "unchecked", source: "pointer" \| "keyboard" \| "api" }` | After commit per the `commitOn` option. |
...
`lifelock:checkbox:*` CustomEvents and to set the native
...
- **State-wash token substitution.** The Figma source for the
  ...
  LifeLock has no `--color-inverse-secondary` alias; this unit
  substitutes `--color-text-secondary` (resolving to
  `--color-off-black` under the LifeLock mode pick) at 20 % / 30 %
