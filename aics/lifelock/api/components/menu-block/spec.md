...
|---|---|---|
| `lifelock:menu-block:open` | `{ root }` | Popup opens |
| `lifelock:menu-block:close` | `{ root }` | Popup closes |
| `lifelock:menu-block:select` | `{ root, item }` | Row activated (click / Enter / Space) |
...
| Enter / Space | Activate focused item (`lifelock:menu-block:select`) |
