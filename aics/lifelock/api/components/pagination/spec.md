...
One unit, three Types: **nav** (numbered pager), **select** (compact page listbox), **jump** (numeric entry). Current page paints `--color-signal-info-subtle` / `--color-signal-info`. `initPaginations` wires click / Enter and emits `lifelock:pagination:change`.
...
|---|---|---|
| `lifelock:pagination:change` | `{ page, root, source }` | Page changes |
