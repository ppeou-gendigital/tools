...
`.c-toggle` Type=`link` (labels + track), `segmented` (bordered primary segment), `pill` (capsule with optional discount-label). `initToggle` emits `lifelock:toggle:change`.
...
|---|---|---|
| `lifelock:toggle:change` | `{ selected, root, source }` | Selection changes |
